...
--1)Вывести количество абитуриентов, сдавших ЕГЭ по каждому предмету, максимальное, минимальное и среднее значение баллов по предмету ЕГЭ, Информация отсортирована по названию предмета в алфавитном порядке
...
--2)Вывести образовательные программы, для которых минимальный балл ЕГЭ по каждому предмету больше или равен 40 баллам.Информация отсортирована по названию программы.
SELECT name_program
FROM program
...
--3)Вывести образовательные программы, на которые для поступления необходимы предмет «Информатика» и «Математика»
SELECT name_program
FROM program p
INNER JOIN program_subject ps ON p.program_id = ps.program_id
...
--4)Вывести название образовательной программы и фамилию тех абитуриентов, которые подавали документы на эту образовательную программу, но не могут быть зачислены на нее. 
...
--5)Вывести название  программы, поступивших на программу "Прикладная механика" и их общее количество баллов. При этом необходимо учесть: 
